...
#        time.sleep(random.randint(1,2))
        ...
        try:
            ...
            content.append(article.content)
            times.append(str(article.datetime))
            comment.append(article.pushes.simple_expression)
...
def crawl_ptt_page_auto_comment(Board_Name ='KoreaDrama' ,
                            page_num= 1):
    ...
    for i in range(page_num):
        ...
        comment_all = pd.DataFrame()

        #開始爬文（頁數）
        # 抓該板的文章
        latest_page = Board(Board_Name, start-i)
        ...
        for summary in latest_page:
            ...
            try:
                ...
                # 串成dataframe
                comment = pd.DataFrame(article.pushes.simple_expression)
                
                # 加入文章網址來源、版名與標題
                comment['文章網址來源'] = 'https://www.ptt.cc' + article.url
                comment['文章標題'] =  article.title
                comment['版名'] = Board_Name

                # 然後concat所有文章
                comment_all = pd.concat([comment,comment_all], axis = 0)
        ...
        listt.append(comment_all)

    listtdf = pd.concat(listt)

    return listtdf 

# g = crawl_ptt_comment(Board_Name,
#                             start,
#                             5
#                             )
# def crawl_ptt_comment(Board_Name,
#                             start,
#                             page 
#                             ):
